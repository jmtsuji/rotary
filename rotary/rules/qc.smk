...
rule run_fastqc_long:
    ...
    output:
        ...
        raw_long_reads_fastqc=temp(expand("{{sample}}/qc/qc_stats/long/{{sample}}_long{end}", end=FASTQC_OUTPUTS)),
        qced_long_reads_fastqc=temp(expand("{{sample}}/qc/qc_stats/long/{{sample}}_nanopore_qc{end}",end=FASTQC_OUTPUTS)),
    conda:
        "../envs/qc.yaml"
    log:
...
rule run_fastq_short:
    ...
    output:
        ...
        raw_short_reads=temp(expand("{{sample}}/qc/qc_stats/short/{{sample}}_{direction}{end}", direction=FASTQ_DIRECTIONS, end=FASTQC_OUTPUTS)),
        qced_short_reads=temp(expand("{{sample}}/qc/qc_stats/short/{{sample}}{file_type}{direction}{end}", file_type=QC_SHORT_FILE_TYPES, direction=FASTQ_DIRECTIONS, end=FASTQC_OUTPUTS)),
        short_contamination_filter=temp(expand("{{sample}}/qc/qc_stats/short/{{sample}}_filter_{direction}{end}", direction=FASTQ_DIRECTIONS, end=FASTQC_OUTPUTS)) if CONTAMINANT_REFERENCE_GENOMES else []
    conda:
        "../envs/qc.yaml"
    log:
